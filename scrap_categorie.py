...
#fonction envoie requête - récupère et parse données HTML
#retrouve datas dans code HTML - télécharge image
...
for y in categories_urls:
    print(
        f'Catégorie disponible : {y.replace("catalogue/category/books/", "").replace("/index.html", "")}\n'
    )
...
with open(
    f"Données-{categorie_choisie}-{str(date.today())}.csv",
    "w",
    encoding="utf-8",
    newline="",
) as file:
